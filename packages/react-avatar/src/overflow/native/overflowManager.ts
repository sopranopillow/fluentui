...
export class OverflowManager {
  private onUpdateItemVisiblity: NonNullable<ObserveOptions['onUpdateItemVisibility']> = () => null;
  /**
  ...
  private overflowDirection: NonNullable<ObserveOptions['overflowDirection']> = 'end';
  private overflowAxis: NonNullable<ObserveOptions['overflowAxis']> = 'horizontal';
  ...
  private overflowGroups: Record<string, { visibleItemIds: Set<string>; invisibleItemIds: Set<string> }> = {};
  /**
  ...
  public observe(container: HTMLElement, options: ObserveOptions = {}) {
    const { padding, overflowAxis, overflowDirection, minimumVisible, onUpdateItemVisibility } = options;
    ...
    this.onUpdateItemVisiblity = onUpdateItemVisibility ?? this.onUpdateItemVisiblity;
  ...
  private getOffsetSize(el: HTMLElement) {
    return this.overflowAxis === 'horizontal' ? el.offsetWidth : el.offsetHeight;
  }
  ...
  private processOverflowItems(availableSize: number) {
    ...
    if (this.visibleItemQueue.peek() !== visibleTop || this.invisibleItemQueue.peek() !== invisibleTop) {
  ...
  private dispatchOverflowUpdate() {
    ...
    const visibleItems = visibleItemIds.map(itemId => this.overflowItems[itemId]);
    const invisibleItems = invisibleItemIds.map(itemId => this.overflowItems[itemId]);
  ...
  private initInvisibleItemQueue() {
    return new PriorityQueue<string>((a, b) => {
      ...
      const positionStatusBit =
        this.overflowDirection === 'end' ? Node.DOCUMENT_POSITION_FOLLOWING : Node.DOCUMENT_POSITION_PRECEDING;

      // equal priority, use DOM order
      if (itemA.element.compareDocumentPosition(itemB.element) & positionStatusBit) {
        return -1;
      } else {
        return 1;
      }
  ...
  private initVisibleItemQueue() {
    return new PriorityQueue<string>((a, b) => {
      ...
      const positionStatusBit =
        this.overflowDirection === 'end' ? Node.DOCUMENT_POSITION_PRECEDING : Node.DOCUMENT_POSITION_FOLLOWING;
